...
&nbsp;
  ...
  private async readFile(subDir: string, circuitId: K, ext: string): Promise&lt;Buffer&gt; {
    ...
    <span class="missing-if-branch" title="if path not taken" >I</span>if (rel === '..' || rel.startsWith(`..${path.sep}`) || path.isAbsolute(rel)) {
<span class="cstat-no" title="statement not covered" >      throw new Error(`Invalid circuitId: ${JSON.stringify(circuitId)}`);</span>
    }
