...
var compactRuntime = require('@midnight-ntwrk/midnight-js-protocol/compact-runtime');
...
var Contract = require('@midnight-ntwrk/midnight-js-protocol/compact-js/effect/Contract');
var ledger = require('@midnight-ntwrk/midnight-js-protocol/ledger');
var onchainRuntime = require('@midnight-ntwrk/midnight-js-protocol/onchain-runtime');
var ContractAddress = require('@midnight-ntwrk/midnight-js-protocol/platform-js/effect/ContractAddress');
...
const unprovenTxFromContractUpdates = async (updateAndSignFn) => {
    return midnightJsTypes.Transaction.fromParts(midnightJsNetworkId.getNetworkId(), undefined, undefined, ledger.Intent.new(midnightJsUtils.ttlOneHour()).addMaintenanceUpdate(await updateAndSignFn()));
};
...
const createZswapOutput = ({ coinInfo, recipient }, encryptionPublicKeyResolver, segmentNumber = 0) => {
    if (!recipient.is_left) {
        return ledger.ZswapOutput.newContractOwned(coinInfo, segmentNumber, recipient.right);
    }
    ...
    return ledger.ZswapOutput.new(coinInfo, segmentNumber, recipient.left, encryptionPublicKey);
};
...
const segmentForMatch = (matches, partitionedTranscript, errorContext) => {
    const [guaranteed, fallible] = partitionedTranscript;
    if (guaranteed !== undefined && matches(guaranteed))
        return GUARANTEED_SEGMENT_NUMBER;
    if (fallible !== undefined && matches(fallible))
    ...
    if (guaranteed !== undefined && fallible !== undefined) {
        throw new Error(`${errorContext} not present in either segment of the partitioned transcript. ` +
...
const segmentForCommitment = (commitment, partitionedTranscript) => segmentForMatch((t) => t.effects.claimedShieldedReceives.includes(commitment) ||
    t.effects.claimedShieldedSpends.includes(commitment), partitionedTranscript, `Shielded commitment ${commitment}`);
const segmentForNullifier = (nullifier, partitionedTranscript) => segmentForMatch((t) => t.effects.claimedNullifiers.includes(nullifier), partitionedTranscript, `Shielded nullifier ${nullifier}`);
...
const bucketToOffer = (bucket) => mergeOffers(unprovenOfferFromMap(bucket.inputs, ledger.ZswapOffer.fromInput), unprovenOfferFromMap(bucket.outputs, ledger.ZswapOffer.fromOutput), unprovenOfferFromMap(bucket.transients, ledger.ZswapOffer.fromTransient));
/**
 ...
 */
const zswapStateToSegmentedOffer = (zswapLocalState, encryptionPublicKeyOrResolver, addressAndChainStateTuple, partitionedTranscript = [undefined, undefined]) => {
    ...
    for (const output of zswapLocalState.outputs) {
        if (output.recipient.is_left) {
            ...
            const commitment = ledger.coinCommitment(output.coinInfo, output.recipient.left);
            ...
            const probe = ledger.ZswapOutput.newContractOwned(output.coinInfo, GUARANTEED_SEGMENT_NUMBER, contractAddress);
            ...
            const finalOutput = segment === GUARANTEED_SEGMENT_NUMBER
                ? probe
                : ledger.ZswapOutput.newContractOwned(output.coinInfo, segment, contractAddress);
    ...
    for (const qualifiedCoinInfo of zswapLocalState.inputs) {
        ...
        if (guaranteedCandidate !== undefined || fallibleCandidate !== undefined) {
            ...
            buckets[transientSegment].transients.set(serializedCoinInfo, ledger.ZswapTransient.newFromContractOwnedOutput(qualifiedCoinInfo, transientSegment, candidateOutput));
            buckets[transientSegment].outputs.delete(serializedCoinInfo);
            continue;
        }
        midnightJsUtils.assertDefined(addressAndChainStateTuple, `Wallet-owned input requires a chain state for ZswapInput.newContractOwned`);
        midnightJsUtils.assertDefined(rehashedChainState, `Wallet-owned input requires a chain state for ZswapInput.newContractOwned`);
        ...
        const probeInput = ledger.ZswapInput.newContractOwned(qualifiedCoinInfo, GUARANTEED_SEGMENT_NUMBER, addressAndChainStateTuple.contractAddress, rehashedChainState);
        ...
        buckets[segment].inputs.set(serializedCoinInfo, segment === GUARANTEED_SEGMENT_NUMBER
            ? probeInput
            : ledger.ZswapInput.newContractOwned(qualifiedCoinInfo, segment, addressAndChainStateTuple.contractAddress, rehashedChainState));
    }
    return {
 ...
 */
const zswapStateToOffer = (zswapLocalState, encryptionPublicKeyOrResolver, addressAndChainStateTuple) => zswapStateToSegmentedOffer(zswapLocalState, encryptionPublicKeyOrResolver, addressAndChainStateTuple).guaranteed;
 ...
 * limitations under the License.
 */
const toLedgerContractState = (contractState) => ledger.ContractState.deserialize(contractState.serialize());
...
const createUnprovenLedgerDeployTx = (contractState, zswapLocalState, encryptionPublicKey) => {
    const contractDeploy = new ledger.ContractDeploy(toLedgerContractState(contractState));
    return [
        contractDeploy.address,
        fromLedgerContractState(contractDeploy.initialState),
        midnightJsTypes.Transaction.fromParts(midnightJsNetworkId.getNetworkId(), zswapStateToOffer(zswapLocalState, encryptionPublicKey), undefined, ledger.Intent.new(midnightJsUtils.ttlOneHour()).addDeploy(contractDeploy))
    ];
};
...
const createUnprovenLedgerCallTx = (circuitId, contractAddress, initialContractState, zswapChainState, partitionedTranscript, privateTranscriptOutputs, input, output, nextZswapLocalState, encryptionPublicKey) => {
    ...
    const intent = ledger.Intent.new(midnightJsUtils.ttlOneHour()).addCall(new ledger.ContractCallPrototype(contractAddress, circuitId, op, partitionedTranscript[0], partitionedTranscript[1], privateTranscriptOutputs, input, output, ledger.communicationCommitmentRandomness(), circuitId));
    const guaranteedOutputs = extractUserAddressedOutputs(partitionedTranscript[0]);
    if (guaranteedOutputs.length > 0) {
        intent.guaranteedUnshieldedOffer = ledger.UnshieldedOffer.new([], guaranteedOutputs, []);
    }
    const fallibleOutputs = extractUserAddressedOutputs(partitionedTranscript[1]);
    if (fallibleOutputs.length > 0) {
        intent.fallibleUnshieldedOffer = ledger.UnshieldedOffer.new([], fallibleOutputs, []);
    }
    ...
    return midnightJsTypes.Transaction.fromPartsRandomized(midnightJsNetworkId.getNetworkId(), segmentedOffers.guaranteed, segmentedOffers.fallible, intent);
};
...
class TransactionContextImpl {
    ...
    [MergeUnsubmittedCallTxData](circuitId, callData, privateStateId) {
        ...
        contractState.data = new onchainRuntime.ChargedState(callData.public.nextContractState);
