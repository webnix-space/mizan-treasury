 ...
 */
class IndexerFormattedError extends IndexerError {
    errors;
    /**
 ...
 */
class IndexerQueryError extends IndexerError {
 ...
 */
class IndexerDataError extends IndexerError {
    context;
    constructor(context) {
    ...
    static formatMessage(context) {
        switch (context.kind) {
            ...
            case 'missing-identifier':
                return (`Transaction missing identifier for contract action at address ${context.contractAddress}` +
 ...
 */
class IndexerSubscriptionDataError extends IndexerError {
    missingField;
 ...
 */
class IndexerProviderConfigError extends IndexerError {
    constructor(message) {
        super(message);
...
const hasContractAction = (data) => data.contractAction != null;
const maybeThrowQueryError = (result) => {
    if (result.error) {
        throw new IndexerQueryError(result.error.message, { cause: result.error });
    }
    return result;
};
const withCompleteQueryData = () => Rx__namespace.pipe(Rx__namespace.filter((result) => {
    if (result.error)
        throw new IndexerQueryError(result.error.message, { cause: result.error });
    return result.dataState === 'complete';
}), 
// Safe: dataState === 'complete' guarantees data is Complete<A> which defaults to A
Rx__namespace.map((result) => result.data));
const withValidFetchData = () => Rx__namespace.pipe(Rx__namespace.map((result) => {
    if (result.errors && result.errors.length > 0) {
        throw new IndexerFormattedError(result.errors);
    }
    return result.data;
}), Rx__namespace.filter((data) => data != null));
...
const deserializeZswapState = (s) => ledger.ZswapChainState.deserialize(toByteArray(s));
const deserializeTransaction = (s) => ledger.Transaction.deserialize('signature', 'proof', 'binding', toByteArray(s));
const deserializeLedgerParameters = (s) => ledger.LedgerParameters.deserialize(toByteArray(s));
/**
 * The default time (in milliseconds) to wait between queries when polling.
 */
const DEFAULT_POLL_INTERVAL = 1000;
// Assumes that the block exists.
const blockOffsetToBlock$ = (apolloClient) => (offset) => apolloClient
    .subscribe({
    ...
    .pipe(withValidFetchData(), Rx__namespace.map((data) => {
...
const transactionIdToTransaction$ = (apolloClient) => (identifier) => apolloClient
    .watchQuery({
    query: TX_ID_QUERY,
    variables: {
        offset: { identifier }
    ...
    .pipe(withCompleteQueryData(), Rx__namespace.filter((data) => data.transactions.length !== 0), Rx__namespace.map((data) => ({
    height: data.transactions[0].block.height
})), Rx__namespace.concatMap(blockOffsetToBlock$(apolloClient)), Rx__namespace.concatMap(({ transactions }) => Rx__namespace.from(transactions)));
 ...
 */
const correlateDeployTxId = (contractAddress, contractActions, identifiers) => {
...
const toFinalizedDeployTxData = (contractAddress, transaction) => ({
...
const contractAddressToLatestBlockOffset$ = (apolloClient) => (contractAddress) => apolloClient
    .watchQuery({
    ...
    .pipe(withCompleteQueryData(), Rx__namespace.filter((data) => data.contractAction !== null), Rx__namespace.map((data) => {
    const contract = data.contractAction;
    return contract.transaction.block.height;
}), Rx__namespace.take(1), Rx__namespace.map((height) => ({ height })));
// Assumes block already exists
const blockOffsetToContractState$ = (apolloClient) => (contractAddress) => (offset) => apolloClient
    .subscribe({
...
}), Rx__namespace.map(deserializeContractState));
const waitForContractToAppear = (apolloClient) => (contractAddress) => (offset) => apolloClient
    .watchQuery({
    ...
    .pipe(withCompleteQueryData(), Rx__namespace.filter(hasContractAction), Rx__namespace.map((data) => data.contractAction.state), Rx__namespace.take(1));
const waitForBlockToAppear = (apolloClient) => (offset) => apolloClient
    .watchQuery({
    query: BLOCK_QUERY,
    variables: {
        offset
    ...
    .pipe(withCompleteQueryData(), Rx__namespace.filter((data) => data.block !== null), Rx__namespace.take(1));
const waitForUnshieldedBalancesToAppear = (apolloClient) => (contractAddress) => apolloClient
    .watchQuery({
    ...
    .pipe(withCompleteQueryData(), Rx__namespace.filter(hasContractAction), Rx__namespace.map((data) => {
    ...
    return [];
}), Rx__namespace.take(1));
const blockOffsetToUnshieldedBalances$ = (apolloClient) => (contractAddress) => (offset) => apolloClient
    .subscribe({
    ...
    return [];
}), Rx__namespace.map(toUnshieldedBalances));
const indexerPublicDataProviderInternal = (queryURL, subscriptionURL, webSocketImpl = ws__namespace.WebSocket) => {
    ...
    midnightJsUtils.warnIfInsecureRemoteUrl(queryURL, 'indexer query URL');
    midnightJsUtils.warnIfInsecureRemoteUrl(subscriptionURL, 'indexer subscription URL');
    // Construct the Apollo client.
    const link = new http.HttpLink({ fetch, uri: queryURL });
    ...
    const apolloLink = core.from([retryLink, link]);
    const apolloClient = new core.ApolloClient({
        link: core.split(({ query }) => {
            const definition = utilities.getMainDefinition(query);
            return definition.kind === 'OperationDefinition' && definition.operation === 'subscription';
        }, new subscriptions.GraphQLWsLink(graphqlWs.createClient({ url: subscriptionURL, webSocketImpl })), apolloLink),
    ...
    return {
        ...
        async watchForDeployTxData(contractAddress) {
            return Rx__namespace.firstValueFrom(apolloClient
                .watchQuery({
            ...
            }), Rx__namespace.filter(isRegularTransaction), Rx__namespace.map((transaction) => toFinalizedDeployTxData(contractAddress, transaction))));
        },
        async watchForTxData(txId) {
            return Rx__namespace.firstValueFrom(apolloClient
                ...
                .pipe(withCompleteQueryData(), Rx__namespace.filter((data) => data.transactions.length !== 0), Rx__namespace.map((data) => data.transactions[0]), Rx__namespace.filter(isRegularTransaction), Rx__namespace.map((transaction) => ({
                tx: deserializeTransaction(transaction.raw),
                status: toTxStatus(transaction.transactionResult),
                txId,
                txHash: transaction.hash,
...
exports.IndexerDataError = IndexerDataError;
exports.IndexerError = IndexerError;
exports.IndexerFormattedError = IndexerFormattedError;
exports.IndexerProviderConfigError = IndexerProviderConfigError;
exports.IndexerQueryError = IndexerQueryError;
exports.IndexerSubscriptionDataError = IndexerSubscriptionDataError;
exports.correlateDeployTxId = correlateDeployTxId;
exports.indexerPublicDataProvider = indexerPublicDataProvider;
exports.isRegularTransaction = isRegularTransaction;
exports.toSegmentStatus = toSegmentStatus;
exports.toSegmentStatusMap = toSegmentStatusMap;
