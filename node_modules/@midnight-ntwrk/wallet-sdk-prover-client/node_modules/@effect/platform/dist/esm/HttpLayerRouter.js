 ...
 * @category HttpApi
 */
export const addHttpApi = (api, options) => HttpApiBuilder.Router.unwrap(Effect.fnUntraced(function* (router_) {
  const router = yield* HttpRouter;
  ...
  const context = yield* Effect.context();
  const apiEndpointKeys = new Set();
  for (const groupKey of Object.keys(api.groups)) {
    const group = api.groups[groupKey];
  ...
  for (const route of router_.routes) {
    if (!apiEndpointKeys.has(`${route.method} ${route.path}`)) {
      continue;
    }
    ...
    routes.push(makeRoute({
      ...route,
      handler: apiMiddleware(Effect.mapInputContext(route.handler, input => Context.merge(context, input)))
    }));
  }
